...
- `extension/detect.js` `compile(config).find(text)`: unit tests in `tests/unit/`, run with `npm run test:unit`.
...
Ask before adding a test at a new seam. Test through these interfaces, not through internals of `extension/content.js`.

Run `npm test` before every commit.

## Landing page

`site/` is the landing page, built with Astro and Tailwind CSS and deployed to GitHub Pages at https://veil-ce.jnahian.me/ by `.github/workflows/deploy-site.yml`. It has its own `package.json`. Run `npm run build` in `site/` before you commit changes there. The page imports its images from `extension/icons/` and `store/assets/`, so run `npm run store:assets` first when the extension UI changes.
